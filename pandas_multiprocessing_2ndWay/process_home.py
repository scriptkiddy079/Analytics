...
def func(x):
    ...
    x["house_age"] = json.dumps(rec["house_age"])
    x["lot_size"] = json.dumps(rec["lot_size"])
    return x
...
def process(df):
    ...
    df.to_sql("housing_aggs", schema="market", if_exists='append', con=prod_engine, index=False)
